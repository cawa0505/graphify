...
# ── Config file support (~/.graphify/config.json) ───────────────────────────
# Allows API keys and backend preference without polluting the shell env.
# Format:
#   {
#     "backend": "openai",
...
#   }
_GRAPHIFY_CONFIG: dict | None = None


def _load_config() -> dict:
    """Load ~/.graphify/config.json once and cache it."""
    global _GRAPHIFY_CONFIG
    if _GRAPHIFY_CONFIG is not None:
        return _GRAPHIFY_CONFIG
    config_path = Path.home() / ".graphify" / "config.json"
    if config_path.is_file():
        try:
            data = json.loads(config_path.read_text(encoding="utf-8"))
            if isinstance(data, dict):
                _GRAPHIFY_CONFIG = data
                return _GRAPHIFY_CONFIG
        except Exception:
            pass
    _GRAPHIFY_CONFIG = {}
...
def _config_api_key(backend: str) -> str:
    """Return current API key for backend from config.json, or empty string.
    Supports key rotation: api_key can be a string or list of strings.
    New structure: providers.<backend>.api_key
    Fallback: api_keys.<backend> (flat)
    """
    keys = _config_api_keys(backend)
    if not keys:
        return ""
    idx = _key_rotator.get(backend, 0)
    return keys[idx % len(keys)]


def _config_api_keys(backend: str) -> list[str]:
    """Return all API keys for backend (for rotation). Always returns a list."""
    config = _load_config()
    providers = config.get("providers", {}).get(backend, {})
    raw = providers.get("api_key", "")
    if not raw:
        raw = config.get("api_keys", {}).get(backend, "")
    if isinstance(raw, list):
        return [str(k) for k in raw if k]
    return [str(raw)] if raw else []


# Module-level key rotation state: {backend: current_index}
_key_rotator: dict[str, int] = {}


def _rotate_api_key(backend: str) -> str:
    """Advance to next API key for backend and return it. Returns empty string if no keys."""
    keys = _config_api_keys(backend)
    if len(keys) <= 1:
        return keys[0] if keys else ""
    current = _key_rotator.get(backend, 0)
    nxt = (current + 1) % len(keys)
    _key_rotator[backend] = nxt
    print(
        f"[graphify] rotated to API key #{nxt + 1}/{len(keys)} for {backend}",
        file=sys.stderr,
    )
    return keys[nxt]


def _config_backend() -> str | None:
    """Return preferred backend from config.json, or None."""
    return _load_config().get("backend")


def _config_model(backend: str) -> str | None:
    """Return model override for backend from config.json, or None.
    New structure: providers.<backend>.model
    Fallback: models.<backend> (flat)
    """
    config = _load_config()
    providers = config.get("providers", {}).get(backend, {})
    model = providers.get("model")
    if not model:
        model = config.get("models", {}).get(backend)
    return model


def _config_base_url(backend: str) -> str | None:
    """Return base_url override for backend from config.json, or None.
    New structure: providers.<backend>.base_url
    Fallback: base_urls.<backend> (flat)
    """
    config = _load_config()
    providers = config.get("providers", {}).get(backend, {})
    url = providers.get("base_url")
    if not url:
        url = config.get("base_urls", {}).get(backend)
    return url


def _config_extraction(key: str, default=None, backend: str | None = None):
    """Return extraction config value (chunk_size, max_concurrency, max_completion_tokens).
    Priority: providers.<backend>.extraction.<key> → extraction.<key> → top-level <key>
    """
    config = _load_config()
    # 1. Per-provider override
    if backend:
        prov = config.get("providers", {}).get(backend, {})
        val = prov.get("extraction", {}).get(key)
        if val is not None:
            return val
    # 2. Global extraction config
...
# Apply config.json base_url overrides to built-in backends
_config_base_urls = _load_config().get("base_urls", {})
for _bname, _burl in _config_base_urls.items():
    if _bname in BACKENDS and isinstance(_burl, str) and _burl:
        BACKENDS[_bname]["base_url"] = _burl
...
def _sanitize_fragment(result: dict) -> dict:
    """Sanitize the returned fragment dictionary so it only contains valid node/edge structures."""
    for key in ("nodes", "edges", "hyperedges"):
        ...
        if not isinstance(val, list):
            result[key] = []
            continue
        result[key] = [item for item in val if isinstance(item, dict)]
    return result
...
def _get_backend_api_key(backend: str) -> str:
    ...
    Priority: ~/.graphify/config.json api_keys[backend] → env vars.
    """
    config_key = _config_api_key(backend)
    if config_key:
        return config_key
...
def _default_model_for_backend(backend: str) -> str:
    ...
    Priority: config.json models[backend] → env var → backend default.
    """
    config_model = _config_model(backend)
    if config_model:
        return config_model
    cfg = BACKENDS[backend]
...
def _call_openai_compat(
    ...
    *,
    ...
    # Retry on 429 rate limits. Two strategies:
    # 1. RPM rate limit (e.g. "retry in 58s") → sleep and retry with same key
    # 2. Daily quota exhaustion ("RESOURCE_EXHAUSTED" / "exceeded your current
    #    quota") → rotate to next API key immediately (no sleep) and recreate
    #    client, so the fresh key's daily quota is used.
    import re as _re
    import time as _time
    from openai import RateLimitError as _RateLimitError
    max_retries = int(os.environ.get("GRAPHIFY_429_MAX_RETRIES", "20"))
    for _attempt in range(max_retries + 1):
        try:
            ...
            if _attempt == max_retries:
                raise
            msg = str(e)
            # Detect daily quota exhaustion (RESOURCE_EXHAUSTED / "exceeded
            # your current quota") — rotate key instead of sleeping.
            is_quota_exhausted = "RESOURCE_EXHAUSTED" in msg or "exceeded your current quota" in msg.lower()
            if is_quota_exhausted:
                n_keys = len(_config_api_keys(backend))
                if n_keys > 1:
                    new_key = _rotate_api_key(backend)
                    if new_key:
                        print(
                            f"[graphify] daily quota exhausted — rotated to key #{(_key_rotator.get(backend, 0) + 1)}/{n_keys}",
                            file=sys.stderr,
                        )
                        client = OpenAI(api_key=new_key, base_url=base_url, timeout=_resolve_api_timeout())
                        continue  # retry immediately with new key
            # RPM rate limit — parse retry delay
            wait = None
            m = _re.search(r"retry[^0-9]*(\d+(?:\.\d+)?)\s*s", msg, _re.IGNORECASE)
            if m:
            ...
            wait = max(wait, 1.0)
            print(
                f"[graphify] 429 rate limit (attempt {_attempt + 1}/{max_retries}); "
                f"sleeping {wait:.0f}s and retrying.",
                file=sys.stderr,
            )
            _time.sleep(wait)
...
# ponytail: stubs for partial-marker API that cli.py (newer) imports but this
# older llm.py never adds markers. No-op keeps the cache/graph path working
# without backporting the full marker plumbing.
def _partial_source_files(_result: dict) -> "set[str]":
    """Return source files whose extraction was truncated this run.

    This older llm.py does not emit partial markers, so there is nothing to
    surface — return an empty set so the cache writer treats everything as
    complete (the pre-marker behavior).
    """
    return set()


def _strip_partial_markers(_result: dict) -> None:
    """Remove internal partial-completion markers before graph serialization.

    No markers are ever added here, so there is nothing to strip.
    """
    return None
...
def _call_llm(
    ...
    *,
    ...
    try:
        from openai import OpenAI, RateLimitError as _OARateLimit, APIStatusError as _OAStatus, APIConnectionError as _OAConn, APITimeoutError as _OATimeout
    except ImportError as exc:
        raise ImportError(_backend_pkg_hint("openai", "openai")) from exc
    base_url = cfg["base_url"]
    client = OpenAI(api_key=key, base_url=base_url, timeout=_resolve_api_timeout())
    kwargs: dict = {
        "model": mdl,
        "messages": [{"role": "user", "content": prompt}],
        "max_completion_tokens": max_tokens,
    ...
    max_attempts = _config_extraction("max_retries", 20, backend=backend)
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            if not resp.choices or resp.choices[0].message is None:
                raise ValueError("LLM returned empty or filtered response")
            return resp.choices[0].message.content or ""
        except (_OARateLimit, _OAStatus) as exc:
            if getattr(exc, "status_code", None) not in (429, 503):
                raise
            msg = getattr(exc, "message", "") or str(exc)
            # Parse retry delay from API response
            retry_after = random.uniform(20, 40)
            m = re.search(r"retry[^0-9]*(\d+(?:\.\d+)?)\s*s", msg, re.IGNORECASE)
            if m:
                retry_after = float(m.group(1)) + random.uniform(1, 5)
            # Detect daily quota exhaustion → rotate key
            is_quota = "RESOURCE_EXHAUSTED" in msg or "exceeded your current quota" in msg.lower()
            if is_quota:
                new_key = _rotate_api_key(backend)
                if new_key is not None:
                    print(f"[graphify] daily quota exhausted — rotated to key #{_key_rotator[backend] + 1}/{len(_config_api_keys(backend))} for {backend}", file=sys.stderr)
                    client = OpenAI(api_key=new_key, base_url=base_url, timeout=_resolve_api_timeout())
                    continue
            if attempt < max_attempts:
                print(f"[graphify] {exc.status_code} (attempt {attempt}/{max_attempts}); sleeping {retry_after:.0f}s", file=sys.stderr)
                time.sleep(retry_after)
        except (_OAConn, _OATimeout) as exc:
            if attempt < max_attempts:
                wait = min(5 * attempt, 60)
                print(f"[graphify] connection error (attempt {attempt}/{max_attempts}); retrying in {wait}s: {exc}", file=sys.stderr)
                time.sleep(wait)
            else:
                raise
    raise RuntimeError(f"[graphify] all {max_attempts} attempts exhausted for {backend}")
...
def detect_backend() -> str | None:
    ...
    Priority: config.json "backend" → gemini → kimi → claude → openai → deepseek → azure → bedrock → ollama (last, opt-in).
    ...
    """
    # 1. Explicit config override wins above all
    configured = _config_backend()
    if configured:
        if configured == "azure":
            if _get_backend_api_key("azure") and os.environ.get("AZURE_OPENAI_ENDPOINT"):
                return "azure"
            # config says azure but env incomplete; fall through
        elif configured in BACKENDS:
            return configured
    # 2. Env-based auto-detection
